...
class Scaler:
    ...
    def __init__(
        self, x_data: np.ndarray, y_data: np.ndarray, scale_data: bool, device: str
    ):
        ...
        # must be also scaled
        if self.scale_data:
            self.y_bounds[0, :] = (y_data.min(0) - y_data.mean(0)) / (
                y_data.std(0) + 1e-12 * np.ones((self.y_std.shape))
            )[:]
            self.y_bounds[1, :] = (y_data.max(0) - y_data.mean(0)) / (
                y_data.std(0) + 1e-12 * np.ones((self.y_std.shape))
            )[:]
            self.x_bounds[0, :] = (x_data.min(0) - x_data.mean(0)) / (
                x_data.std(0) + 1e-12 * np.ones((self.x_std.shape))
            )[:]
            self.x_bounds[1, :] = (x_data.max(0) - x_data.mean(0)) / (
                x_data.std(0) + 1e-12 * np.ones((self.x_std.shape))
            )[:]
        ...
        log.info(
            "Datset Info: state min: {} and max: {}, action min: {} and max: {}".format(
                self.x_bounds[0, :],
                self.x_bounds[1, :],
                self.y_bounds[0, :],
                self.y_bounds[1, :],
    ...
    def scale_output(self, y):
        ...
        if self.scale_data:
            out = (y - self.y_mean) / (
                self.y_std + 1e-12 * torch.ones((self.y_std.shape), device=self.device)
            )
            return out.to(torch.float32)
        else:
    ...
    def inverse_scale_output(self, y):
        ...
        if self.scale_data:
            ...
            out = (
                ...
                * (
                    self.y_std
                    + 1e-12 * torch.ones((self.y_std.shape), device=self.device)
                )
                + self.y_mean
            )
            return out
    ...
    def scale_block_push_goal(self, x):
        ...
        if self.scale_data:
            ...
            out = (
                x
                * (x - self.x_mean[[0, 1, 3, 4]])
                / (
                    self.x_std[[0, 1, 3, 4]]
                    + 1e-12
                    * torch.ones((self.x_std[[0, 1, 3, 4]].shape), device=self.device)
                )
            )
            return out
        else:
