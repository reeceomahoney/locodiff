...
class VelocityCommand {
 public:
  ...
  void step(const Eigen::Matrix3d &robotRotation) {
    if (stepsUntilNextSample_-- <= 0) {
      ...
      if (commandMode == commandMode_ &&
          commandMode_ != VelocityCommandMode::Direction &&
  ...
  const Eigen::Vector3d &getVelocityCommand() { return velocityCommand_; }

  void setSeed(const int &seed) { gen_.seed(seed); }

  bool zeroCommandMode() { return commandMode_ == VelocityCommandMode::Zero; }

 private:
  void sampleVelocityCommand() {
    velocityCommand_ = Eigen::Vector3d::NullaryExpr(
  ...
  VelocityCommandMode sampleCommandMode() {
    return static_cast<VelocityCommandMode>(
        commandModeSamplingDistribution_(gen_));
  }
  ...
  void validateVelocityCommand() {
    ...
    if (velocityCommandNorm < limitVelocityMagnitude_) {
      if (velocityCommandNorm < 1e-2) {
        ...
        velocityCommand_ =
            velocityCommand_ * (limitVelocityMagnitude_ / velocityCommandNorm);
      }
    }
  }

  void updateDirectionGoal() {
    directionGoal_ = Eigen::Vector2d::NullaryExpr(
        [&]() { return uniformRealDistribution_(gen_); });
  }
  ...
  void updateVelocityForGoal(const Eigen::Matrix3d &robotRotation) {
    ...
    robotHeadingAngle_ =
        std::atan2(robotRotation.col(0)[1], robotRotation.col(0)[0]);
    headingAngleOffset_ =
        std::atan2(std::sin(desiredHeadingAngle_ - robotHeadingAngle_),
  ...
  int getRandomSignInt() { return randomSignDistribution_(gen_) == 0 ? -1 : 1; }
 ...
 private:
  ...
  std::discrete_distribution<int> commandModeSamplingDistribution_,
      randomSignDistribution_{0.5, 0.5};
  ...
  double desiredHeadingAngle_ = 0., robotHeadingAngle_ = 0.,
         headingAngleOffset_ = 0.;
};
