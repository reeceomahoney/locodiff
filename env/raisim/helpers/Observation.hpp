...
class ObservationHandler {
 private:
  ...
  Eigen::VectorXd generalizedCoordinate_, generalizedVelocity_,
      generalizedForce_;
  ...
  double nominalBaseHeight_ = 0.59;
  // double nominalBaseHeight_ = 0.45;
  // double nominalBaseHeight_ = 0.285;
 ...
 public:
  ObservationHandler() {
    ...
    jointNominalConfig_ << -0.14, 0.81, -0.96, 0.14, 0.81, -0.96, -0.14, -0.81,
        0.96, 0.14, -0.81, 0.96;
    // jointNominalConfig_ << -0.17, 1.25, -1.56,
    //         0.17, 1.25, -1.56,
    //         -0.17, -1.25, 1.56,
    //         0.17, -1.25, 1.56;
    // jointNominalConfig_ << -0.30, 1.90, -2.27,
    //         0.30, 1.90, -2.27,
    //         -0.30, -1.90, 2.27,
    //         0.30, -1.90, 2.27;

    feetNominalPositions_.setZero(12);
    feetNominalPositions_ << 0.38, 0.27, -0.522, 0.38, -0.27, -0.522, -0.38,
        0.27, -0.522, -0.38, -0.27, -0.522;
  ...
  void updateObservation(raisim::ArticulatedSystem *&robot,
                         const Eigen::VectorXd &desiredJointPosition) {
    ...
    quaternionToRotationMatrix(baseQuaternionVerticalComponent_,
                               baseRotationVerticalComponent_);
    ...
    baseLinearVelocity_ =
        baseRotation_.e().transpose() * generalizedVelocity_.segment(0, 3);
    baseAngularVelocity_ =
        baseRotation_.e().transpose() * generalizedVelocity_.segment(3, 3);

    /// Set State Vector
    observation_.segment(0, 3) =
        baseRotation_.e().row(2).transpose();  // Indices 0 - 2: Gravity Axes
    observation_.segment(3, nJoints_) = generalizedCoordinate_.tail(
        nJoints_);  // Indices 3 - 14: Joint Positions
    observation_.segment(15, 3) =
        baseAngularVelocity_;  // Index 15 - 17: Angular Velocity
    observation_.segment(18, nJoints_) =
        generalizedVelocity_.tail(nJoints_);  // Index 18 - 29: Joint Velocities
    observation_.segment(30, 3) =
        baseLinearVelocity_;  // Index 30 - 32: Base Linear Velocity
    observation_.segment(33, 2) = desiredBaseLinearVelocity_.head(
        2);  // Index 33 - 34: Velocity Command Linear
    observation_.segment(35, 1) = desiredBaseAngularVelocity_.tail(
        1);  // Index 35: Velocity Command Angular
    observation_.segment(36, 12) =
        desiredJointPosition -
        generalizedCoordinate_.tail(
  ...
  void updateJointHistory(const Eigen::VectorXd &desiredJointPosition) {
    ...
    jointPositionErrorHistory_.head((historyLength_ - 1) * nJoints_) =
        varJoint_.tail((historyLength_ - 1) * nJoints_);
    jointPositionErrorHistory_.tail(nJoints_) =
        desiredJointPosition - generalizedCoordinate_.tail(nJoints_);
  ...
  void reset(raisim::ArticulatedSystem *&raisim,
             const Eigen::VectorXd &velocityCommand) {
  ...
  Eigen::VectorXd &getObservation() { return observation_; }

  Eigen::VectorXd &getDesiredJointPosition() { return desiredJointPosition_; }

  Eigen::VectorXd &getNominalJointConfiguration() {
    return jointNominalConfig_;
  }

  Eigen::VectorXd &getNominalFeetPositions() { return feetNominalPositions_; }

  Eigen::VectorXd &getJointPositionErrorHistory() {
    return jointPositionErrorHistory_;
  }

  Eigen::VectorXd &getJointVelocityHistory() { return jointVelocityHistory_; }

  [[nodiscard]] int getObservationDim() const { return observationDim_; }

  Eigen::VectorXd &getBaseLinearVelocity() { return baseLinearVelocity_; }

  Eigen::VectorXd &getBaseAngularVelocity() { return baseAngularVelocity_; }

  Eigen::VectorXd &getGeneralizedCoordinate() { return generalizedCoordinate_; }

  Eigen::VectorXd &getGeneralizedVelocity() { return generalizedVelocity_; }

  Eigen::VectorXd &getGeneralizedForce() { return generalizedForce_; }

  raisim::Mat<3, 3> &getBaseRotation() { return baseRotation_; }
  ...
  static void quaternionToRotationMatrix(const raisim::Vec<4> &q,
                                         raisim::Mat<3, 3> &r) {
  ...
  static void quaternionToRotationMatrix(const Eigen::Matrix<double, 4, 1> &q,
