...
class VisualizationHandler {
 ...
 public:
  ...
  void setServer(const std::unique_ptr<raisim::RaisimServer> &server) const {
    ...
    server->addVisualCylinder("heading_velocity", 0.0025, 0.0025, 1., 0., 0.,
                              1.);
    server->addVisualCylinder("lateral_velocity", 0.0025, 0.0025, 0., 1., 0.,
                              1.);
    server->addVisualCylinder("yaw_rate", 0.0025, 0.0025, 0., 0., 1., 1.);
  }

  void updateVelocityVisual(
      raisim::ArticulatedSystem *&robot, ObservationHandler &observationHandler,
    ...
    raisim::rotMatToQuat(baseRotationHeadingComponent_,
                         baseQuaternionHeadingComponent_);
    server->getVisualObject("heading_velocity")
        ->setOrientation(baseQuaternionHeadingComponent_.e());

    headingVisualOffset_ =
        robot->getBaseOrientation() *
        raisim::Vec<3>{
            observationHandler.getDesiredBaseLinearVelocity()[0] * 0.25, 0.,
            0.};
    ...
    server->getVisualObject("heading_velocity")
        ->setCylinderSize(
            0.02, softSign(std::abs(
                      observationHandler.getDesiredBaseLinearVelocity()[0])) *
                      1.5);
    ...
    raisim::rotMatToQuat(baseRotationHeadingComponent_,
                         baseQuaternionHeadingComponent_);
    server->getVisualObject("lateral_velocity")
        ->setOrientation(baseQuaternionHeadingComponent_.e());

    lateralVisualOffset_ =
        robot->getBaseOrientation() *
        raisim::Vec<3>{
            0., observationHandler.getDesiredBaseLinearVelocity()[1] * 0.25,
            0.};
    robot->getBasePosition(basePosition_);
    ...
    server->getVisualObject("lateral_velocity")
        ->setCylinderSize(
            0.02, softSign(std::abs(
                      observationHandler.getDesiredBaseLinearVelocity()[1])) *
                      1.5);
    ...
    server->getVisualObject("yaw_rate")
        ->setCylinderSize(
            0.2 * softSign(std::abs(
                      observationHandler.getDesiredBaseAngularVelocity()[2])),
            0.05);
